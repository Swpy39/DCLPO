...
def linear_single_pair(ppd_datas, pc_datas, alpha):
    ...
    for num, data in enumerate(ppd_datas):
        ...
        for response in prompt_response:
            results[prompt]['responses'].append(response)
        results[prompt]['sft'].append(responses[0])
        for pc_data in pc_datas:
    ...
    # 保存三元组样本以及对应的难度分数
    with open("../dataset/Three-pairs/ultrafeedback(alpha={}).json".format(alpha), 'w',
              encoding='utf-8') as f:
    ...
    with open("../dataset/Three-pairs/ultrafeedback_score_list(alpha={}).json".format(alpha), 'w',
              encoding='utf-8') as f:
